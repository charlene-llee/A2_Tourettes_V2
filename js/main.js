...
// How close (in px) the player's jump has to be to a trap for it to arm.
// Only Stage 2's two pits ever use this (every other stage's trapGround
// entry starts `prefallen`, which skips the arm/trigger step entirely).
// Each block butts up against its pit, so a player who walks into the
// block and jumps from right against it ends up roughly ~150px from the
// trap's center — this needs to comfortably clear that "resting against
// the wall" distance so the jump reliably arms the pit at a reasonable
// distance, without being so wide it triggers from clear across the
// screen.
