...
const LEVEL_1_STAGES = [
...

  {
    ...
    movingPlatforms: [
      ...
      { x: 760, y: 350, width: 200, range: 0, speed: 0, phase: 0 }, // widened, leads toward the mailbox
      { x: 980, y: 230, width: 200, range: 0, speed: 0, phase: 0 }, // widened, leads toward the mailbox
      { x: 1100, y: 140, width: 180, range: 0, speed: 0, phase: 0 }, // landing, widened, holds the mailbox
    ],

    // `flash: true` makes these boxes blink on/off, each on its own
    // rate/phase (flashOn/flashOff/flashPhase — see isHazardVisible() in
    // main.js) so they don't all flicker in lockstep. One sits on each
    // remaining stationary platform along the climb (kept off the two
    // moving platforms so a box is never left floating mid-air when its
    // platform drifts out from under it). They still damage the player
    // the entire time, blink or not.
    hazards: [
      // y:460 platform
      {
        x: 935,
